...
const Verification = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios
      .post(`${LARAVEL_BACKEND_URL}/verifyotp`, {
        email: user.email,
        otp: otp,
      })
      .then(async (res) => {
      ...
      <div className={AuthCSS.wrapForgotpass}>
        ...
        <form className={AuthCSS.forgotpasswordform} onSubmit={handleSubmit}>
          ...
          <div className={AuthCSS.forgotpassInput}>
            <input className={AuthCSS.input100} type="number" name="otp" placeholder="Enter OTP" value={otp} onChange={handleChange} required />
          </div>
