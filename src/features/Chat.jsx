...
const Chat = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			if (user) {
				try {
					const response = await axios.get(`${apiUrl}backend/chat/users.php`);
					const filteredOtherUsers = response.data.filter((userData) => userData !== user.email);
	...
	useEffect(() => {
		const fetchData = async () => {
			if (currentUser) {
				try {
					const response = await axios.post(`${apiUrl}backend/chat/getmessages.php`, {
						sender: user.email,
	...
	const sendMessage = async (e) => {
		...
		await axios
			.post(`${apiUrl}backend/chat/storemessage.php`, {
				sender: user.email,
				receiver: currentUser,
				message: message,
			})
